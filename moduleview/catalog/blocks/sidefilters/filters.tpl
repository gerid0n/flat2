...
<div class="sidebar sec-filter rs-filter-section{if $smarty.cookies.filter} expand{/if}" data-query-value="{$url->get('query', $smarty.const.TYPE_STRING)}">
    ...
    <form method="GET" class="filters rs-filters" action="{urlmake pf=null bfilter=null p=null}" autocomplete="off">
                    ...
                    <div class="detail">
                        ...
                        <div class="filter-fromto">
                            <div class="input-wrapper">
                                <label>{t}от{/t}</label>
                                <input type="text" class="rs-filter-from" name="bfilter[cost][from]" value="{if !$catalog_config.price_like_slider}{$basefilters.cost.from}{else}{$basefilters.cost.from|default:$moneyArray.interval_from}{/if}" data-start-value="{if $catalog_config.price_like_slider}{$moneyArray.interval_from|intval}{/if}">
                            </div>
                            <div class="input-wrapper">
                                <label>{t}до{/t}</label>
                                <input type="text" class="rs-filter-to" name="bfilter[cost][to]" value="{if !$catalog_config.price_like_slider}{$basefilters.cost.to}{else}{$basefilters.cost.to|default:$moneyArray.interval_to}{/if}" data-start-value="{if $catalog_config.price_like_slider}{$moneyArray.interval_to|intval}{/if}">
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="sidebar_menu_buttons">
            ...
            <button type="reset" class="theme-btn_reset rs-clean-filter"><i class="pe-7s-close-circle"></i>{t}Сбросить фильтр{/t}</button>
        </div>
    </form>
</div>