                        ...
                        <h3 class="h3">{t}Контактные данные{/t}</h3>
                            {if !$is_auth}
                                <div class="form-group visible-xs visible-sm">
                                    <input type="radio" id="type-user" name="user_type" value="person" {if $order.user_type=='person'}checked{/if}> <label for="type-user">Частное лицо</label><br>
                                    <input type="radio" id="type-company" name="user_type" value="company" {if $order.user_type=='company'}checked{/if}> <label for="type-company">Компания</label><br>
                                    <input type="radio" id="type-noregister" name="user_type" value="noregister" {if $order.user_type=='noregister'}checked{/if}> <label for="type-noregister">Без регистрации</label><br>
