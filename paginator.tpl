{if $paginator->total_pages>1}
    ...
    <div class="paginator">
        ...
        {if $paginator->page < $paginator->total_pages}
            <a href="{$paginator->getPageHref($paginator->page+1)}" data-page="{$paginator->page+1}" title="следующая страница" class="next">
                <i class="pe-7s-angle-right"></i>
            </a>
        {/if}

        {if $paginator->showLast()}
            <a href="{$paginator->getPageHref($paginator->total_pages)}" data-page="{$paginator->total_pages}" title="последняя страница" class="last">{$paginator->total_pages}</a>
        {/if}
    </div>
{/if}